...
export const GridScan = ({
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const tiltSm = smoothDampFloat(tiltCurrent.current, tiltTarget.current, tiltVel.current, smoothTime, maxSpeed, dt);
      tiltCurrent.current = tiltSm.value;
      tiltVel.current = tiltSm.velocity;

      const yawSm = smoothDampFloat(yawCurrent.current, yawTarget.current, yawVel.current, smoothTime, maxSpeed, dt);
...
function smoothDampFloat(
  ...
  return { value: out, velocity: currentVelocity };
}
